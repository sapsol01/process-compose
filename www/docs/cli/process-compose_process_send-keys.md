## process-compose process send-keys

Send keystroke(s) to an interactive process's stdin

### Synopsis

Send keystroke(s) to a running interactive process's stdin.
...
```
process-compose process send-keys [PROCESS] [KEYS] [flags]
```
